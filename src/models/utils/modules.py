...
def build_action_block_causal_attention_mask(T, H, W, add_tokens=1):
    ...
    for t1 in range(T):
        for t2 in range(max(0, t1 - local_window_time + 1), t1 + 1):
            mask[t1 * N_T : (t1 + 1) * N_T, t2 * N_T : (t2 + 1) * N_T] = (
                mask_block
            )
...
class ACRoPEAttention(nn.Module):
    ...
    def forward(
        ...
        B, N, C = x.size()
        assert T is not None and H is not None and W is not None, (
            "T, H, W must be provided to ACRoPEAttention.forward"
        )
        ...
        if action_tokens > 0:
            ...
            for i in range(action_tokens):
                ...
                kr = k[..., self.d_dim :]
                action_q += [
                    torch.cat([qd, qr], dim=-1).view(
                        B, self.num_heads, T, 1, -1
                    )
                ]
                action_k += [
                    torch.cat([kd, kr], dim=-1).view(
                        B, self.num_heads, T, 1, -1
                    )
                ]
        ...
        if action_tokens > 0:
            ...
            def merge_(tx, ta):
                ...
                ta = ta.view(
                    B, self.num_heads, T, action_tokens, -1
                )  # [B, T, A, D]
                return torch.cat([ta, tx], dim=3).flatten(2, 3)

            q = merge_(q, action_q)  # type: ignore
            k = merge_(k, action_k)  # type: ignore
            v = merge_(v, action_v)  # type: ignore
...
class RoPEAttention(nn.Module):
    ...
    def forward(
        ...
        if mask is not None:
            mask = mask.unsqueeze(1).repeat(1, self.num_heads, 1)
            d_mask, h_mask, w_mask = self.separate_positions(
                mask, H_patches, W_patches
            )
        else:
            if T is None or H_patches is None or W_patches is None:
                mask = torch.arange(
                    int(grid_depth * self.grid_size * self.grid_size),
                    device=x.device,
                )
            else:
                mask = torch.arange(
                    int(T * H_patches * W_patches), device=x.device
                )
            d_mask, h_mask, w_mask = self.separate_positions(
                mask, H_patches, W_patches
            )
